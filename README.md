...
* When per-dependency git commits are enabled, each successful bump runs `go mod tidy`, then commits `go.mod` and `go.sum` as `chore(deps): update MODULE to VERSION`.
